...
The application is built on top of analytics-ready tables generated by the data pipeline.

## Orchestration

**Apache Airflow** is used to orchestrate the entire data pipeline.
It coordinates the execution of the different steps, including data ingestion and data transformations, ensuring that each task runs in the correct order.
It manages the execution of data ingestion tasks and triggers dbt transformations to produce reliable datasets.

At the current stage, the application focuses on demonstrating how analytics-ready data produced by the data pipeline can be consumed by a visualization layer. 
Additional performance metrics and comparisons will be implemented in future iterations.
